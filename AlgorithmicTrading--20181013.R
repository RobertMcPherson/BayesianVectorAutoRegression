...
currency('USD')
init_date <- "2007-12-31" #the date on which we want to intialize the portfolio and account
start_date <- "2009-12-31" #the date from which we want to collect the data
end_date <- "2018-10-12" #the date untill when we want to collect the data
init_equity <- 1000 #initial account equity value
adjustment <- TRUE #TRUE when we want to adjust prices otherwise FALSE

getSymbols(Symbols = "SPY", src = "yahoo", from = start_date, to = end_date, adjust = adjustment)
...
plot_ly(x = df$Date, type="candlestick",
        open = df$SPY.Open, close = df$SPY.Close,
        high = df$SPY.High, low = df$SPY.Low)
...
initPortf(name = portfolio.st,symbols = "SPY",initDate = init_date)

initAcct(name = account.st,portfolios = portfolio.st,initDate = init_date,initEq = init_equity)

initOrders(portfolio = portfolio.st,symbols = "SPY",initDate = init_date)

strategy(strategy.st, store = TRUE)

#SMA
color="red"
chartSeries(SPY$SPY.Close,TA="addSMA(n=40,col=color)") 

#MACD
chartSeries(SPY$SPY.Close, TA="addMACD(fast = 12, slow = 26, signal = 9, histogram = TRUE)")

#RSI
chartSeries(SPY$SPY.Close, TA="addRSI(n=14)")

#Bollinger Bands
sma="SMA"
bands="bands"
color="blue"
chartSeries(SPY$SPY.Close, TA="addBBands(n=20,maType=sma,draw=bands)")
...
currency('USD')
init_date <- "2007-12-31" #the date on which we want to intialize the portfolio and account
start_date <- "2007-12-31" #the date from which we want to collect the data
end_date <- "2018-10-12" #the date untill when we want to collect the data
init_equity <- 1000 #initial account equity value
adjustment <- TRUE #TRUE when we want to adjust prices otherwise FALSE
...
chartSeries(RSI(SPY$SPY.Close,n=7),theme="black",name="RSI n=7")
...
chartSeries(SPY$SPY.Close,TA="addMACD(fast = 12, slow = 26, signal = 9)",theme="black",name="MACD 26-12-9")


mktdata_ind <- applyIndicators(strategy=strategy.st,mktdata=SPY)
...
write.csv(mktdata_sig, file="mktdata_sig.csv")
...
#Per trade stats
knitr::kable(head(perTradeStats(portfolio.st, Symbol = "SPY")))
