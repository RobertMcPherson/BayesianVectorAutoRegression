...
data <- apply(data, 2, rev) #reverse to select 35 most recent years
data <- data[1:35,]
data <- apply(data, 2, rev) #reverse again to descending order

colnames(data) <- c("CPI","shortTermDebtToLongTerm","debtToGDP","GDP","m1Velocity","m2Velocity")

##Import birth data

births <- read.csv("births_interp_no_blanks_young_old_ratio.csv")
length(births[,1])
head(births)

births.2009 <- apply(births, 2, rev)
head(births.2009)
...
length(SeriesData[1,])
#x <- SeriesData[, !(names(SeriesData) %in% paste(dependent.variable))]
SeriesData <- SeriesData[,-10] #remove the dependent variable from the x variables
head(x)
...
#save the dependent variable unscaled
y.unscaled <- y

#save another version of the dependent variable as scaled
y.scaled <- scale(y)
