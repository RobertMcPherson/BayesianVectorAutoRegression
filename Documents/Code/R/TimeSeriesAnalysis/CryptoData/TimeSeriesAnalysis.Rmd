...
time.increments <- unique(x[,time.increment.variable])
#time.increments <- time.increments[sort.list(time.increments, decreasing=FALSE)]#save time increment vector
...
x.colnames <- make.names(colnames(x), unique=TRUE)
...
Run QQ-Plots to visually check the data distributions.
...
forecast_matrix <- function (x, maxlag)
{
    ...
    x_new = NULL
    for (j in 1:p) {
      hw <- HoltWinters(x[,j], gamma=FALSE, beta=FALSE)
      #?HoltWinters
...
write.csv(file="x_new_whitened.csv",x=x_new_whitened)
...
#cvfitlm.coef.lambda.min.prewhitened <- coef(cvfitlm.prewhitened, s = "lambda.min")
cvfitlm.coef.lambda.1se.prewhitened <- coef(cvfitlm.prewhitened, s = .20)
...
#colnames(cvfitlm.coef.lambda.prewhitened) <- c("Variable","Coefficient")
...
try(coef.indices.lambda.1se <- which(cvfitlm.coef.lamdbda.1se[,"Coefficient"] != 0))

try(if(coef.indices.lambda.1se > 1) {
...
} else {print("No variables other than the intercept are significant leading indicators.")})
...
if(length(important.vars.matrix[,1]) >= length(important.vars.matrix[1,])){
  biplot(princomp(important.vars.matrix), cex=.8, arrow.len=.1)
} else {biplot(princomp(t(important.vars.matrix)), cex=.8, arrow.len=.1)}
