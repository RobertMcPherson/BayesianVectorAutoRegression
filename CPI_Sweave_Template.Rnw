...
We can see in Figure~\ref{fig:BVARInfluencersCPI} that all the predictor variables tend to be relatively strong influencers of each other, but do not have as much influence on inflation as measured by CPI.  Rather, CPI tends to be more of an influencer for many of the other variables, especially economic ones.
